...
export default function NoteSettingsPanel({
  ...
  const handleBgSelect = useCallback((color: string) => {
    void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    onBackgroundChange(color);
  }, [onBackgroundChange]);

  const handleFontSelect = useCallback((size: number) => {
    void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.colorGrid}>
              {NOTE_BACKGROUNDS.map((bg) => (
                ...
                >
                  {backgroundColor === bg.color && (
                    <Check size={14} color={isLightBg(bg.color) ? '#333' : '#fff'} />
                  )}
                </Pressable>
              ))}
            </View>
            <View style={styles.colorLabels}>
              {NOTE_BACKGROUNDS.map((bg) => (
                <Text
                  key={bg.id}
                  style={[styles.colorLabel, backgroundColor === bg.color && styles.colorLabelActive]}
                  numberOfLines={1}
                >
                  {bg.label}
                </Text>
            ...
            <View style={styles.optionRow}>
              {FONT_SIZES.map((fs) => (
                <Pressable
                  key={fs.id}
                  style={[styles.optionChip, fontSize === fs.size && styles.optionChipActive]}
            ...
            <View style={styles.optionRow}>
              {LINE_SPACINGS.map((ls) => (
                <Pressable
                  key={ls.id}
                  style={[styles.optionChip, lineSpacing === ls.value && styles.optionChipActive]}
                  onPress={() => handleSpacingSelect(ls.value)}
                >
                  <Text
                    style={[styles.optionChipText, lineSpacing === ls.value && styles.optionChipTextActive]}
                  >
                    {ls.label}
                  </Text>
                </Pressable>
