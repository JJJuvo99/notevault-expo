...
const CODE_LANGUAGES_GROUPED = [
  {
    label: 'Popular',
    languages: ['javascript', 'typescript', 'python', 'java', 'html', 'css', 'sql', 'bash', 'json'],
  },
  {
    label: 'Systems',
...
export default function BlockInsertMenu({ visible, onClose, onInsert }: BlockInsertMenuProps) {
          ...
          <Pressable onPress={(e) => e.stopPropagation()}>
            ...
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>
                {showLanguagePicker ? 'Code Language' : showTableConfig ? 'Table Size' : 'Insert Block'}
              </Text>
              {showLanguagePicker && (
                <Pressable
                    ...
                    <View key={group.label} style={styles.langGroup}>
                      ...
                      <View style={styles.langGroupGrid}>
                        {group.languages.map((lang) => (
                          <Pressable
                            key={lang}
                            style={({ pressed }) => [styles.languageChip, pressed && styles.languageChipPressed]}
              ...
              <View style={styles.tableConfig}>
                <View style={styles.tableConfigRow}>
                  ...
                  <View style={styles.tableStepperRow}>
                    <Pressable
                      ...
                      onPress={() => {
                        const v = Math.max(1, (parseInt(tableRows, 10) || 3) - 1);
                    ...
                    <Pressable
                      style={styles.stepperBtn}
                      onPress={() => {
                        const v = Math.min(20, (parseInt(tableRows, 10) || 3) + 1);
                        setTableRows(String(v));
                        void Haptics.selectionAsync();
                      }}
                    >
                ...
                <View style={styles.tableConfigRow}>
                  ...
                  <View style={styles.tableStepperRow}>
                    <Pressable
                      style={styles.stepperBtn}
                      onPress={() => {
                        const v = Math.max(1, (parseInt(tableCols, 10) || 3) - 1);
                    ...
                    <Pressable
                      style={styles.stepperBtn}
                      onPress={() => {
                        const v = Math.min(10, (parseInt(tableCols, 10) || 3) + 1);
                        setTableCols(String(v));
                        void Haptics.selectionAsync();
                      }}
                    >
                ...
                <View style={styles.tablePreview}>
                  <Text style={styles.tablePreviewLabel}>
                    {parseInt(tableRows, 10) || 3} × {parseInt(tableCols, 10) || 3} table
                  </Text>
                </View>
                <Pressable style={styles.tableInsertBtn} onPress={handleTableInsert}>
              ...
              <>
                ...
                <ScrollView style={styles.blockList} showsVerticalScrollIndicator={false}>
                  {filteredBlocks.map((block) => (
                    <Pressable
                      key={block.id}
                      style={({ pressed }) => [styles.blockItem, pressed && styles.blockItemPressed]}
