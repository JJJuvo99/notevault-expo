...
export default function JournalScreen() {
  ...
  console.log(
    "[JournalScreen] Rendering for date:",
    selectedDate,
    "isToday:",
    isToday,
    "habits:",
  ...
  const saveJournal = useCallback(() => {
    ...
    if (journalEntry) {
      ...
      addJournalEntry(
        ...
        journalMood,
        journalContent.trim(),
        journalGratitude.trim() || undefined,
      );
    }
    setEditingJournal(false);
  ...
  const moodData = journalEntry
    ? MOOD_OPTIONS.find((m) => m.key === journalEntry.mood)
    : null;
        ...
        <View style={styles.section}>
          ...
          {showAddHabit && (
            <View style={styles.addHabitRow}>
              ...
              />
              <Pressable
                style={styles.addHabitConfirm}
                onPress={handleAddHabit}
                testID="btn-confirm-habit"
              >
          ...
          <View style={styles.sectionHeader}>
            ...
            {journalEntry && !editingJournal && (
              <View style={styles.entryActions}>
                <Pressable
                  onPress={openJournalEditor}
                  style={styles.editBtn}
                  testID="btn-edit-journal"
                >
          ...
          {editingJournal ? (
            <View style={styles.journalEditor}>
                ...
                {MOOD_OPTIONS.map((m) => (
                  ...
                  >
                    {getMoodIcon(
                      m.key,
                      20,
                      journalMood === m.key ? m.color : Colors.textMuted,
                    )}
                    <Text
                      style={[
                        styles.moodLabel,
                        journalMood === m.key && { color: m.color },
                      ]}
                    >
                      {m.label}
                    </Text>
              ...
              <View style={styles.editorButtons}>
                <Pressable
                  style={styles.cancelEditorBtn}
                  onPress={() => setEditingJournal(false)}
                >
            ...
            <View style={styles.journalView}>
              <View style={styles.moodDisplay}>
                {getMoodIcon(
                  journalEntry.mood,
                  24,
                  moodData?.color ?? Colors.text,
                )}
                <Text
                  style={[
                    styles.moodDisplayLabel,
                    { color: moodData?.color ?? Colors.text },
                  ]}
                >
                  Feeling {moodData?.label.toLowerCase() ?? journalEntry.mood}
                </Text>
              </View>
              <Text style={styles.journalViewContent}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.recentList}>
              {recentEntries.map((entry) => {
                const entryMood = MOOD_OPTIONS.find(
                  (m) => m.key === entry.mood,
                );
                  ...
                  >
                    <View
                      style={[
                        ...
                        {
                          backgroundColor: entryMood?.color ?? Colors.textMuted,
                        },
                      ]}
                    />
                    ...
                    {getMoodIcon(
                      entry.mood,
                      16,
                      entryMood?.color ?? Colors.textMuted,
                    )}
                  </Pressable>
                );
              })}
