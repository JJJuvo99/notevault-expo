...
export default function NotebookDetailScreen() {
  ...
  const notebookNotes = useMemo(() => getNotebookNotes(id ?? ''), [getNotebookNotes, id]);

  console.log('[NotebookDetail] Rendering notebook:', notebook?.name, 'with', notebookNotes.length, 'notes');

  const handleNotePress = useCallback((noteId: string) => {
    console.log('[NotebookDetail] Opening note:', noteId);
    router.push({ pathname: '/note-editor', params: { noteId, notebookId: id } });
  }, [router, id]);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]} testID="notebook-detail-screen">
      <Stack.Screen options={{ headerShown: false }} />

      <View style={styles.header}>
        <Pressable onPress={() => router.back()} style={styles.backBtn} testID="btn-back">
          <ArrowLeft size={22} color={Colors.text} />
        </Pressable>
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle} numberOfLines={1}>{notebook?.name ?? 'Notebook'}</Text>
